...
{   
    ...
    public class Questrade : IBrokerage
    {
        ...
        private bool _isQuoteStreamSuccess;
        private bool _isNotifiSubSuccess;

        private HttpClient authClient;
        private HttpClient apiClient;
        
        public QuestradeWebsocket notificationClient;
        ...
        public event EventHandler<APIOrderNotificationRecievedArg> OnOrderNotificationRecieved;
        public event EventHandler<APIOrderExecNotificationRecievedArg> OnOExecutionNotificationRecieved;
        ...
        public Questrade(string token = "")
        {
            ...
            _isNotifiSubSuccess = notificationClient != null && notificationClient.State == WebSocketState.Open;
            _isQuoteStreamSuccess = quoteStreamClient != null && quoteStreamClient.State == WebSocketState.Open;
        ...
        /// <returns></returns>
        public async Task Authenticate(string clientId)
        {
            HttpResponseMessage resp;

            //_auth.refresh_token = refreshToken;

            //string authUrl = $"https://login.questrade.com/oauth2/authorize?client_id=<client_id>&response_type=code&redirect_uri=https://www.example.com";
        ...
        /// </summary>
        /// <returns></returns>
        public void SubscribeToNotificationsAsync(CancellationTokenSource cancellationTokenSource)
        {
            CancellationToken ct = cancellationTokenSource.Token;
            Task.Run(() =>
            {
        ...
        private void NotificationClient_OnReceive(object sender, Websocket.Events.MessageEventArg e)
        {
            if (_isNotifiSubSuccess)
            {
                if (e.message.Contains("executions"))
                {
                    ...
                    OnOExecutionNotificationRecieved(this, arg);
                }
                else if (!e.message.Contains("success"))
                {
                    ...
                    OnOrderNotificationRecieved(this, arg);
                }
            }
            else
            {
